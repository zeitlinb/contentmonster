...
>
> _Last updated: 2026-07-07 (killed the book/KDP pipeline — D1)._
...
- ~~**Book-publishing pipeline** — books to sell on Amazon (KDP).~~ **KILLED 2026-06-27**
  (not viable; abandoned). Superseded by an **authority-content** lead-gen direction —
  scoped in memory (`[[project-authority-content-strategy]]`), kept out of canon. See §3 D1.
...
|---|----------|--------|-------------|
| D1 | ~~Book-publishing pipeline (KDP)~~ | **KILLED 2026-06-27** — selling books on Amazon/KDP is not viable; abandoned. Research retained as artifacts; direction moved to authority content (lead-gen), scoped in memory, not canon. | [`../book-publishing/`](../book-publishing/) (archived research) |
...
| D3 | **Headless QA auth** — Pass 2 is "we are Claude" interactively; needs `ANTHROPIC_API_KEY` when run headless/CI. | DECIDED in design (require key, fail-loud on refusal). _The book pipeline it referenced is **killed** (D1); the auth decision still applies to any future prose/QA pipeline._ | [`../book-publishing/pipeline-design-D1.md`](../book-publishing/pipeline-design-D1.md) |
...
typed stubs. The book pipeline (D1) is **killed** (2026-06-27); the remaining deferred build is
the ad pipeline's Drive-sync + `generate-scene` CLIs (see [`../runbooks/google-drive-setup.md`](../runbooks/google-drive-setup.md)).

## Changelog
- 2026-07-07 — **Killed the book-publishing (Amazon/KDP) pipeline (D1)** — not viable,
  abandoned 2026-06-27. Research retained as artifacts; direction moved to authority
  content (lead-gen), tracked in memory, not canon.
